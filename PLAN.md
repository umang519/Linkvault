...
**Progress: 14 of 14 screens built.** All routes now render real screens (no `PlaceholderScreen` stand-ins left) — Onboarding, Sign in, Sign up, Forgot password, Home, Search, Browse, Category detail, Add link, Link detail, Edit link, Saved, Settings, Filters.

- [x] Shared components — all built: `LinkRow` (detailed + compact, now composed from `LetterMark` + `StatusBadge` rather than inlining them), `TabBar` (5 tabs + floating Save block), `LetterMark`, `StatusBadge`, `Chip` (filter/tag pill, solid/outline/dashed), `SegmentedControl` (status row, appearance row), `Skeleton`/`SkeletonRow` (pulse animation via RN's `Animated`), `Toast` (Undo/View), `BottomSheet` (now used by `CategoryPickerSheet`), `Dialog` (Delete confirmation / Duplicate / Status picker / Sort picker — two position variants), `Toggle` (favorite/favorites-only switch, added this pass — RN's `Switch` can't honor "no border-radius"), `DeleteLinkDialog`, `StatusPickerDialog`, `CategoryPickerSheet` (the latter three new, shared between the screens that need them rather than duplicated). All in `src/components/`.
- [x] Mock data layer: `src/mocks/links.ts` + `src/mocks/categories.ts`. Mutations (add/edit/delete/favorite/status) now go through a `links` Redux slice (`src/store/linksSlice.ts`), seeded from `mocks/links.ts`, so they're visible across every screen rather than each screen holding a static copy — `useMockLinks` and the already-built Search/Browse/Category-detail screens were pointed at the same slice.
- [x] `src/lib/linkFilters.ts` (AND-combining filter engine shared by Search and Filters, per PROJECT.md §5.2 — built with Smart Collections reuse in mind) and `src/lib/urlNormalize.ts` (validation/normalization for Add Link, per §7.2).
- First run — **done**: Onboarding, Sign in, Sign up, Forgot password (see above)
- Home — **done**: greeting + 4 retrieval counts + search field, loading (skeleton rows via `useMockLinks`), empty-library state, recent feed, saved-toast (wired to `route.params.justSaved`, populated by Add Link on save). Offline/net-error states not yet built (need real connectivity detection — low priority until apps/api exists to actually go offline from)
- Search — **done**: idle, live results (title/url/description/notes/tags per §5.1), no-results state; now also accepts a `filters` route param from the Filters screen (AND-combined with the text query, active-filter count shown on the FILTER button).
- Browse — **done**: Categories → Category detail; Tags; Collections placeholder (V2, unchanged).
- [x] Mock category data + shared count helpers (`linkCountForCategory`, `tagCounts`) — both now accept a `links` override so they read live data.
- [x] Add link: URL-required form, Share-to-app sheet (double-tap-to-save on an already-selected suggestion chip, per §14.9), in-progress/metadata-failed/invalid-URL states (metadata fetch is simulated — no `apps/metadata` service yet, that's V2), duplicate-detected dialog (client-side normalized-URL check against the `links` slice).
- [x] Link detail (ruled meta table, Open Link via `Linking.openURL`, Share via RN's `Share` API, tap-to-change status), Edit link (same field order as Add, prefilled), Delete confirmation (shared `DeleteLinkDialog`, opened from both).
- [x] Saved tab: Favorites (search + category chips, empty state explaining Favorite vs. Status), Status/"To read" list with swipe-to-advance (`react-native-gesture-handler`'s `Swipeable`, first real usage of the already-installed dependency) **and a long-press fallback menu** with the same status options (accessibility — see §14.9).
- [x] Settings (You tab): Appearance segmented control is wired for real to `ThemeProvider`'s `setSetting`; other rows (account, defaults, sync, import/export, security) are non-functional placeholders per §14.8, matching the `TODO(auth)` convention already used by the auth screens.
- [x] Filters: category/sub-category/tags/status/date/source/favorites/sort, footer shows a live result count computed against the `links` slice, hands its result back to Search via a route param.
- [ ] Dark mode: verify the token swap (PROJECT.md §14.2) renders correctly across all of the above rather than styling dark mode screen-by-screen — not yet manually verified on-device for the 6 screens built this pass.
- [ ] Responsive check at small (375×667) and large (430×932) phone sizes, per the design's own check — not yet manually verified for the 6 screens built this pass.
